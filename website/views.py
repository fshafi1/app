...
openai.api_key = "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"
...
def home_page():
    ...
    for match in matches:
        ...
        match_dict['is_recommended'] = match.zodiacSign in zodiac_list[current_user.zodiacSign]
        matches_dicts.append(match_dict)

    session['matches'] = matches_dicts
    #print("TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT", session['matches'])
    
    return render_template('home_page.html', user=current_user, matches=matches_dicts)
...
@views.route('/block_user', methods=['POST'])
@login_required
def block_user():
    blocked_user_id = request.form.get('blocked_user_id')

    liked_profile = LikedDislikedProfile.query.filter_by(user_id=current_user.id, profile_id=blocked_user_id).first()

    if liked_profile is None:
        # If there's no existing record, create a new one with the blocked status
        blocked_profile = LikedDislikedProfile(
            user_id=current_user.id,
            profile_id=blocked_user_id,
            liked=False,
            blocked=True
        )
        db.session.add(blocked_profile)
    else:
        # If there's an existing record, update the blocked status
        liked_profile.blocked = True
...
@views.route('/blocked_profiles')
@login_required
def blocked_profiles():
    blocked_profiles = LikedDislikedProfile.query.filter_by(user_id=current_user.id, blocked=True).all()
    blocked_user_ids = [profile.profile_id for profile in blocked_profiles]
    blocked_users = User.query.filter(User.id.in_(blocked_user_ids)).all()
    return render_template('blocked_profiles.html', blocked_users=blocked_users)
